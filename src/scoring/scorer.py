"""Framework-agnostic scoring pipeline.

CLIP ViT-B/32 embedding -> Ridge regression score + Logistic Regression
classifier -> UMAP/KMeans style cluster -> percentile rank against the
training set. No Streamlit or FastAPI imports here — this module is shared
by both the Streamlit app (src/app/shared.py) and the FastAPI service
(src/api/main.py).
"""
...
@dataclass
class Resources:
    """Everything the scoring pipeline needs, loaded once and reused across requests."""
    clip: CLIPModel
    processor: CLIPProcessor
    scorer: object
    classifier: object
    device: str
    training_preds: np.ndarray
...
def load_resources() -> Resources:
    """Load CLIP, the trained scorer/classifier, and the training dataset. Cached — runs once per process."""
    ...
    return Resources(
        clip=clip, processor=processor, scorer=scorer, classifier=classifier,
        device=device, training_preds=training_preds, embeddings=embeddings,
        df=df, umap_3d=umap_3d, cluster_centroids=cluster_centroids,
    )


def run_pipeline(img: Image.Image, resources: Resources) -> dict:
    """Run the full pipeline and return a rich, internal result dict (numpy arrays included).

    Used by the Streamlit app, which needs the raw embedding/similarity data to render
    the "Similar Photos" and "Visual Style Map" tabs. For a clean, JSON-safe result, use
    score_image() instead.
    """
    clip, processor, scorer, classifier, device = (
        resources.clip, resources.processor, resources.scorer, resources.classifier, resources.device
    )
    training_preds, embeddings, df, umap_3d, cluster_centroids = (
        resources.training_preds, resources.embeddings, resources.df,
        resources.umap_3d, resources.cluster_centroids,
    )
...
def _prepare_image(image: Image.Image) -> Image.Image:
    """Normalise an uploaded image: fix EXIF rotation, convert to RGB, cap size."""
...
def score_image(image: Image.Image) -> dict:
    """Score a single photo. Returns a plain, JSON-serialisable dict — the public entry
    point for the FastAPI service (and anything else that just wants a score, not the
    full internal pipeline state).
    """
    image = _prepare_image(image)
    resources = load_resources()
    r = run_pipeline(image, resources)

    quality_verdict = "high" if r["clf_pred"] == 1 else "low"
    confidence = float(r["clf_proba"] if r["clf_pred"] == 1 else 1 - r["clf_proba"])

    return {
        "aesthetic_score": r["aesthetic_score"],
        "percentile": round(r["percentile"], 1),
        "score_label": r["score_lbl"],
        "quality_verdict": quality_verdict,
        "confidence": round(confidence, 3),
        "cluster_id": r["cluster_id"],
        "cluster_name": r["cluster_name"],
        "attributes": [
            {"label": label, "similarity": round(sim, 3)} for label, sim in r["attributes"]
        ],
        "genre_scores": [
            {"label": label, "score": round(score, 3)} for label, score in r["genre_scores"]
        ],
    }
